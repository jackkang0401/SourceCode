...
struct __CFRunLoop {
    ...
    pthread_t _pthread;                 // runloop 对应的线程, 和线程是一一对应的
    uint32_t _winthread;
    CFMutableSetRef _commonModes;       // 存储的是字符串，记录所有标记为 common 的 mode
    CFMutableSetRef _commonModeItems;   // 存储所有 commonMode 的 item (source，timer，observer 数据结构被统称为 mode item)
    CFRunLoopModeRef _currentMode;      // 当前运行的 mode, 如果切换 mode，只能退出后重新进入, 一个时间点只能在一种 mode
...
struct __CFRunLoopSource {  // source、observer、timer
    CFRuntimeBase _base;
...
struct __CFRunLoopObserver {
    ...
    CFIndex _order;			/* immutable */
    CFRunLoopObserverCallBack _callout;	/* immutable */         // 回调函数
...
struct __CFRunLoopTimer {
    ...
    CFMutableSetRef _rlModes;                           // 存放所有包含该 timer 的 modeName，一个 timer 可能会在多个 mode 中
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    uint64_t startTSR = mach_absolute_time(); // 获取系统启动后的 CPU 运行时间，用于控制超时时间

    if (__CFRunLoopIsStopped(rl)) {     // 如果 RunLoop 是 stop 状态，return
        __CFRunLoopUnsetStopped(rl);
        return kCFRunLoopRunStopped;
    } else if (rlm->_stopped) {         // 如果 mode 是 stop 状态，return
        rlm->_stopped = false;
        return kCFRunLoopRunStopped;
    }
    
    mach_port_name_t dispatchPort = MACH_PORT_NULL; // mach 端口，在内核中，消息在端口之间传递，初始为0
    Boolean libdispatchQSafe = pthread_main_np() && ((HANDLE_DISPATCH_ON_BASE_INVOCATION_ONLY && NULL == previousMode) || (!HANDLE_DISPATCH_ON_BASE_INVOCATION_ONLY && 0 == _CFGetTSD(__CFTSDKeyIsInGCDMainQ)));
    // libdispatchQSafe && 主线程 && runloop 是主线程的 runloop && 该 mode 是 commonMode，则给 mach 端口赋值为主线程收发消息的端口
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    CHECK_FOR_FORK();
    if (__CFRunLoopIsDeallocating(rl)) return kCFRunLoopRunFinished;                        // 要释放，返回
    __CFRunLoopLock(rl);
    CFRunLoopModeRef currentMode = __CFRunLoopFindMode(rl, modeName, false);                // 根据 modeName 找到本次运行的 mode (如果不存在不会创建)
    if (NULL == currentMode || __CFRunLoopModeIsEmpty(rl, currentMode, rl->_currentMode)) { // 没找到 || mode 中没有注册任何事件
        Boolean did = false;
        if (currentMode) __CFRunLoopModeUnlock(currentMode);
        __CFRunLoopUnlock(rl);
        return did ? kCFRunLoopRunHandledSource : kCFRunLoopRunFinished;                    // 返回
    }
    volatile _per_run_data *previousPerRun = __CFRunLoopPushPerRunData(rl);     // 可以理解为挂起当前状态
    CFRunLoopModeRef previousMode = rl->_currentMode;
    rl->_currentMode = currentMode;
    int32_t result = kCFRunLoopRunFinished;

	if (currentMode->_observerMask & kCFRunLoopEntry ) __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopEntry);// 1.通知 observer 即将进入 runloop
	result = __CFRunLoopRun(rl, currentMode, seconds, returnAfterSourceHandled, previousMode);
	if (currentMode->_observerMask & kCFRunLoopExit ) __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopExit);  // 10.通知 observer 已退出 runloop

    __CFRunLoopModeUnlock(currentMode);
    __CFRunLoopPopPerRunData(rl, previousPerRun);                               // 可以理解为恢复之前状态
	rl->_currentMode = previousMode;
    __CFRunLoopUnlock(rl);
    return result;
}
// 用 kCFRunLoopDefaultMode 启动
