...
typedef struct {
    ...
    void	(*schedule)(void *info, CFRunLoopRef rl, CFStringRef mode); // 当 source 加入到 mode 触发的回调
    void	(*cancel)(void *info, CFRunLoopRef rl, CFStringRef mode);   // 当 source 从 runloop 中移除时触发的回调
    void	(*perform)(void *info);     // 当 source 事件被触发时的回调，使用 CFRunLoopSourceSignal 方式触发
...
#else
    void *	(*getPort)(void *info);     // 当 source 被添加到mode中的时候，从这个函数中获得具体 mach_port_t
